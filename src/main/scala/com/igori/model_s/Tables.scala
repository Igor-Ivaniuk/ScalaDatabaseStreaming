...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = FlywaySchemaHistory.schema ++ Review.schema
  ...
  class FlywaySchemaHistory(_tableTag: Tag) extends profile.api.Table[FlywaySchemaHistoryRow](_tableTag, Some("sds"), "flyway_schema_history") {
   ...
   *  @param rating Database column rating SqlType(INT)
   *  @param reviewText Database column review_text SqlType(TEXT), Default(None) */
  case class ReviewRow(id: Int, customerId: Int, rating: Int, reviewText: Option[String] = None)
  /** GetResult implicit for fetching ReviewRow objects using plain SQL queries */
  implicit def GetResultReviewRow(implicit e0: GR[Int], e1: GR[Option[String]]): GR[ReviewRow] = GR{
    prs => import prs._
    ReviewRow.tupled((<<[Int], <<[Int], <<[Int], <<?[String]))
  }
  /** Table description of table review. Objects of this class serve as prototypes for rows in queries. */
  class Review(_tableTag: Tag) extends profile.api.Table[ReviewRow](_tableTag, Some("sds"), "review") {
    def * = (id, customerId, rating, reviewText) <> (ReviewRow.tupled, ReviewRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(customerId), Rep.Some(rating), reviewText)).shaped.<>({r=>import r._; _1.map(_=> ReviewRow.tupled((_1.get, _2.get, _3.get, _4)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(INT), AutoInc, PrimaryKey */
    val id: Rep[Int] = column[Int]("id", O.AutoInc, O.PrimaryKey)
    /** Database column customer_id SqlType(INT) */
    val customerId: Rep[Int] = column[Int]("customer_id")
    /** Database column rating SqlType(INT) */
    val rating: Rep[Int] = column[Int]("rating")
    /** Database column review_text SqlType(TEXT), Default(None) */
    val reviewText: Rep[Option[String]] = column[Option[String]]("review_text", O.Default(None))
  }
